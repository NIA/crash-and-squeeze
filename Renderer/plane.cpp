...
void plane(float length, float width, Vertex *res_vertices, Index *res_indices, const float4 & color)
{
    ...
    for( int i = -PLANE_STEPS_PER_HALF_SIDE; i <= PLANE_STEPS_PER_HALF_SIDE; ++i )
    {
        ...
        {
            res_vertices[vertex] = Vertex( float3( x_step*i, y_step*j, 0), color, normal);
