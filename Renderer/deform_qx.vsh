...
void sum_iter(float4 pos, float3 normal, inout float3 sum_pos, inout float3 sum_normal, int ci)
{
    ...
    // Now add quadratic deformation:
    float3 offset_quad = {offset.x*offset.x, offset.y*offset.y, offset.z*offset.z};
    float3 offset_mix  = {offset.x*offset.y, offset.y*offset.z, offset.z*offset.x};
    sum_pos += mul(offset_quad, (float3x3)clus_mx_quad[ci]);
    sum_pos += mul(offset_mix,  (float3x3)clus_mx_mix[ci]);
}
